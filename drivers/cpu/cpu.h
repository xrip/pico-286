...
#ifdef OPS_286_386
#define cyrix_write_seg_descriptor cyrix_write_seg_descriptor_2386
#define cyrix_load_seg_descriptor cyrix_load_seg_descriptor_2386
#else
...
// TODO:
#define fatal(...) 
inline static void pci_reset() {}
inline static void dma_reset() {}
inline static void dma_set_at(int is286) {}
inline static void device_reset_all(int d) {}
#define DEVICE_KBC 1
#define DEVICE_SOFTRESET 2
#define DEVICE_ALL 0xffffffff  /* match all devices */
inline static void ppi_reset() {}
inline static void picint() {}
inline static void pc_speed_changed() {}
inline static void gdbstub_cpu_init() {}
inline static void io_handler(int set, uint16_t base, int size,
...
inline static  void timer_set_new_tsc(uint64_t new_tsc){}
inline static  void picintc(uint32_t m){}
#define MIN(x, y) ((x > y) ? (y) : (x))
#ifdef USE_DYNAREC
int      cpu_use_dynarec;     /* CPU recompiler enabled */
#else
#define cpu_use_dynarec 0
#endif
extern void portout(uint16_t portnum, uint16_t value);
extern void portout16(uint16_t portnum, uint16_t value);
extern void portout32(uint16_t portnum, uint32_t value);
extern uint16_t portin(uint16_t portnum);
extern uint16_t portin16(uint16_t portnum);
extern uint32_t portin32(uint16_t portnum);
#define outb portout
#define outw portout16
#define outl portout32
#define inb portin
#define inw portin16
#define inl portin32
...
extern struct i8259_s i8259_controller;
...
#define picinterrupt nextintr
#define pic_irq_ack nextintr

extern size_t mem_size;
void resub_cycles(int old_cycles);
#define is_pcjr 0
#define config_changed 0
extern void     plat_munmap(void *ptr, size_t size);
extern void    *plat_mmap(size_t size, uint8_t executable);
#define GDBSTUB_MEM_ACCESS(...)
#define GDBSTUB_MEM_ACCESS_FAST(...)
