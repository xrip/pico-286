...
irq_pending(int nec_hlt)
{
    ...
    temp = (nmi && nmi_enable && nmi_mask) || ((cpu_state.flags & T_FLAG) && !noint) || (i_flag && i8259_controller.interrupt_request_register & (~i8259_controller.interrupt_mask_register) && !noint);
...
check_interrupts(int nec_hlt)
{
    ...
    if (irq_pending(nec_hlt)) {
        ...
        if (i_flag && i8259_controller.interrupt_request_register & (~i8259_controller.interrupt_mask_register) && !noint) {
            repeating = 0;
            completed = 1;
            ovr_seg   = NULL;
            wait_cycs(3, 0);
