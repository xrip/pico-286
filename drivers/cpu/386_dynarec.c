...
#    endif
        ...
        if (smi_line)
            ...
            CPU_BLOCK_END();
        else if ((cpu_state.flags & I_FLAG) && i8259_controller.interrupt_request_register & (~i8259_controller.interrupt_mask_register) && !cpu_end_block_after_ins)
            CPU_BLOCK_END();
    }
...
#    endif
            ...
            if (nmi && nmi_enable && nmi_mask)
                CPU_BLOCK_END();
            if ((cpu_state.flags & I_FLAG) && i8259_controller.interrupt_request_register & (~i8259_controller.interrupt_mask_register) && !cpu_end_block_after_ins)
                CPU_BLOCK_END();
...
#    else
                nmi = 0;
#    endif
            } else if ((cpu_state.flags & I_FLAG) && i8259_controller.interrupt_request_register & (~i8259_controller.interrupt_mask_register)) {
                vector = picinterrupt();
                if (vector != -1) {
#    ifndef USE_NEW_DYNAREC
                    oldcs = CS;
...
/// TODO:
static const int timer_target = 0;
void
