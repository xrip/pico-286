    ...
    for (int y = 0; y < 480; y++) {
        ...
        switch (videomode) {
            ...
            case 0x10: /* EGA 640x350 16 color */ {
                ...
                // Apply CRTC vram_offset. Assuming vram_offset is a byte offset.
                uint8_t *base_ptr = VIDEORAM + vram_offset;

                for (int x = 0; x < 640; x++) {
                    uint32_t offset_in_plane = (y * 80) + (x / 8); // 80 bytes per row (640 pixels / 8 bits_per_byte)
                    uint8_t bit_pos = 7 - (x % 8);

                    // With vga_plane_size at 64KB (0x10000), offset_in_plane (max ~28000) will be < vga_plane_size.
                    // The check `if (offset_in_plane >= vga_plane_size)` is no longer needed to truncate rendering.

                    // Boundary check against total VIDEORAM_SIZE (now 256KB)
                    // Max access: offset_in_plane + vga_plane_size * 3 = ~28000 (0x6D60) + 3 * 65536 (0x30000) = ~220KB (0x36D60)
                    // This should be well within VIDEORAM_SIZE (256KB = 0x40000)
                    if ((offset_in_plane + vga_plane_size * 3) >= VIDEORAM_SIZE) {
                         *pixels++ = 0; // Should not happen with correct sizes, indicates an issue if it does.
                         continue;
                    }

                    uint8_t bit0 = (base_ptr[offset_in_plane + vga_plane_size * 0] >> bit_pos) & 1;
                    uint8_t bit1 = (base_ptr[offset_in_plane + vga_plane_size * 1] >> bit_pos) & 1;
                    uint8_t bit2 = (base_ptr[offset_in_plane + vga_plane_size * 2] >> bit_pos) & 1;
                    uint8_t bit3 = (base_ptr[offset_in_plane + vga_plane_size * 3] >> bit_pos) & 1;
            ...
            case 0x12: /* EGA 640x480 16 color */ {
                ...
                for (int x = 0; x < 640; x++) {
                    uint32_t offset_in_plane = (y * 80) + (x / 8); // 80 bytes per row
                    int bit_pos = 7 - (x % 8); // Bit position within the byte

                    // Max offset_in_plane for 640x480: (479 * 80) + (639/8) = 38320 + 79 = 38399 (approx 0x9600)
                    // This is < vga_plane_size (64KB).
                    // Max access: offset_in_plane + vga_plane_size * 3 = ~38KB + 3 * 64KB = ~38KB + 192KB = ~230KB (0xE6000)
                    // This should be within VIDEORAM_SIZE (256KB = 0x40000, wait, 230KB is 0x39600. This is fine.)
                    // Oh, calculation error: 230KB is 0x39600. VIDEORAM_SIZE is 0x40000. Max access is fine.
                     if ((offset_in_plane + vga_plane_size * 3) >= VIDEORAM_SIZE) {
                         *pixels++ = 0; // Should not happen with correct sizes
                         continue;
                    }

                    uint8_t color_val = ((base_ptr[offset_in_plane + vga_plane_size * 0] >> bit_pos) & 1);
                    color_val |= ((base_ptr[offset_in_plane + vga_plane_size * 1] >> bit_pos) & 1) << 1;
                    color_val |= ((base_ptr[offset_in_plane + vga_plane_size * 2] >> bit_pos) & 1) << 2;
                    color_val |= ((base_ptr[offset_in_plane + vga_plane_size * 3] >> bit_pos) & 1) << 3;
                    *pixels++ = vga_palette[color_val];
                }
                break;
            }
            case 0x13: {
